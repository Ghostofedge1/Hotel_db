...
#*********************  End ***********************************
#*********************  guest data managment functions  ***********************************

def table_update(rows):
   tree.delete(*tree.get_children())
   for dt in rows:
      tree.insert('', 'end', values=(dt[0], dt[1], dt[2], dt[3], dt[4]))
...
def update_guest():
   ...
   if updateoptionss.get() == "Room number" or updateoptionss.get() == "number of days" or updateoptionss.get() == "Bill update":
      try:
         int(entery_updateds.get())
         if updateoptionss.get() == "number of days" and int(entery_updateds.get()) > 30  :
            entery_updateds.configure(bg="#EE0000")
            check = False
            messagebox.showwarning(title='WARNING!', message='you cant stay more than 30 days !')
   ...
   if updateoptionss.get() == "Room number" :
      try:
         sql = "SELECT * FROM guests WHERE ROOM_REG='%d'" % (int(entery_updateds.get()))
         cursor.execute(sql)
         result = cursor.fetchone()

         if cursor.rowcount > 0:
            check = False
            messagebox.showinfo(title='info', message='This room is already taken !')
         else:
            check = True
      ...
      if messagebox.askyesno("confirm update?", "are you sure you want to update the guest's data?"):
         try:
            ...
            if updateoptionss.get() == "Room number":
               ...
               sql = "SELECT EMAIL FROM guests WHERE ID=%d" % (id_guest)
               cursor.execute(sql)
               result = cursor.fetchone()
               email_msg = """ Hello  , \n  your room has changed to """ + entery_updateds.get() + """\n have a nice day """
               welcoming_email('Feedback', result[0], email_msg)
               ...
               cursor.execute(sql)
               result = cursor.fetchone()
               email_msg = """ Hello  , \n  your number of days has been extended  to """ + entery_updateds.get() + """days \n have a nice day """
               welcoming_email('Feedback', result[0], email_msg)
               entery_updateds.delete(0, END)
               clear()

            elif updateoptionss.get() == "Bill update":
               sql = "UPDATE guests SET BILL = '%d' WHERE ID = '%d'" % \
                     (int(entery_updateds.get()), id_guest)
               cursor.execute(sql)
               db_conn.commit()
               messagebox.showinfo(title='info', message='updated successfully !')
               ...
               cursor.execute(sql)
               result = cursor.fetchone()
               email_msg = """ Hello  , \n  your Bill  has been updated  to """ + entery_updateds.get() + """\n have a nice day """
               welcoming_email('Feedback', result[0], email_msg)
               entery_updateds.delete(0, END)
               clear()
...
def checkout_guest():
   ...
   if (len(guest_room) == 0 & len(guest_name) == 0):
      ...
      if messagebox.askyesno("confirm checkout?", "are you sure (" + guest_name + ") want to CHECKOUT?"):
         ...
         cursor.execute(sql)
         result = cursor.fetchone()
         email_msg = """ Hello , \n  We hope you enjoyed your time in our hotel , don't forget to tell your friends about your wonderful experince ,\n and also don't forget to give us a feed back \nhave a nice day """
         welcoming_email('Feedback', result[0], email_msg)
         query = "DELETE FROM guests WHERE ID=%d " % (id_guest)
         cursor.execute(query)
         db_conn.commit()
         clear()
         global row_selected
      ...
      if check==True:
         if messagebox.askyesno(title='Confirmation', message='Do you really want to submit ?'):
            ...
            try:
               ...
               email_msg="""\t\t\t\t Welcome to our hotel \t\t\t\t \n we hope that you are gonna have a nice time here \n your ID is """ + str(result[0])
               welcoming_email('Welcome message',email.get(),email_msg)
...
def search_for_a_guest():
   ...
   search_entery.configure(bg=white)
   check = False
   result=""

   if search.get()=="ID":
     try:
        sql = "SELECT * FROM guests WHERE ID='%d'" % ( int(s_num.get()))
        cursor.execute(sql)
        result = cursor.fetchone()
   ...
   if  check_name(wk_name,wk_E1) and check_age(wk_age,wk_E2) and check_wk_addr() and check_email(wk_email,wk_E4) \
      ...
      if messagebox.askyesno(title='Confirmation', message='Do you really want to submit ?'):
         ...
         try:
            ...
            msg = 'your ID is ' + str(result[0])
            messagebox.showinfo(title='Info', message=msg)
            email_msg = """\t\t\t\t Welcome to our hotel \t\t\t\t \n we hope that you enjoy working  with us \n you are going to start on the next week \n your ID is """ + str(result[0])
            welcoming_email('Welcome message', wk_email.get(),email_msg)
...
def update_worker_info():
   ...
   if len(wk_id.get())==0:
      ...
      entery_id1.configure(bg="#f75957")
      check = False
   elif len(wk_id.get())>0:
      try:
         sql = "SELECT * FROM workers WHERE ID='%d'" % (int(wk_id.get()))
         cursor.execute(sql)
         result = cursor.fetchone()
   ...
   if updateoptions1.get() == "Choose":
      messagebox.showwarning(title='WARNING!', message='pls choose an option from the option list')
      check = False

   # updatelist = ["Salary", 'phone number', 'Address', 'Email']
   if len(entery_updated1.get())==0:
      messagebox.showwarning(title='WARNING!', message='Edit box is empty !')
      entery_updated1.configure(bg=war_red)
      check = False
   elif updateoptions1.get()=="Salary" or updateoptions1.get()=="phone number":
      try:
         int(entery_updated1.get())
         if updateoptions1.get()=="phone number" and len(entery_updated1.get()) !=11:
            entery_updated1.configure(bg=war_red)
            check = False
   ...
   if check :
      ...
      entery_id1.configure(bg="#ffffff")
      entery_updated1.configure(bg="#ffffff")
      if updateoptions1.get() == "Salary":
         try:
            if messagebox.askyesno(title='Confirmation', message='Do you really want to update ?'):
               sql = "UPDATE workers SET SALARY = '%d' WHERE ID = '%d'" % (int(entery_updated1.get()), int(wk_id.get()))
               cursor.execute(sql)
               db_conn.commit()
               messagebox.showinfo(title='info', message='updated successfully !')
               sql = "SELECT EMAIL FROM workers WHERE ID=%d" % (int(wk_id.get()))
               cursor.execute(sql)
               result = cursor.fetchone()
               email_msg = """ Hello mate , \n We would like to inform you that we updated your salary  \n your salary now is """ + entery_updated1.get()+""" \n have a nice day """
               welcoming_email('Feedback',result[0],email_msg)
               entery_id1.delete(0, END)
               entery_updated1.delete(0, END)
               updateoptions1.set('Choose')
      ...
      elif updateoptions1.get() == "Email":
         if check_email(wk_update_entery,entery_updated1) :
            try:
               if messagebox.askyesno(title='Confirmation', message='Do you really want to update ?'):
                  sql = "UPDATE workers SET 	EMAIL = '%s' WHERE ID = '%d'" % (entery_updated1.get(), int(wk_id.get()))
                  ...
                  cursor.execute(sql)
                  result = cursor.fetchone()
                  email_msg = """ Hello mate , \n  your new email is """ + entery_updated1.get() + """\n have a nice day """
                  welcoming_email('Feedback',result[0], email_msg)
                  entery_id1.delete(0, END)
                  entery_updated1.delete(0, END)
                  updateoptions1.set('Choose')
      ...
      elif updateoptions1.get() == "phone number":
         if check_mobile(wk_update_entery,entery_updated1):
            try:
               if messagebox.askyesno(title='Confirmation', message='Do you really want to update ?'):
                  sql = "UPDATE workers SET PHONE = '%s' WHERE ID = '%d'" % (entery_updated1.get(), int(wk_id.get()))
                  ...
                  cursor.execute(sql)
                  result = cursor.fetchone()
                  email_msg = """ Hello mate , \n  your  new phone number is """ + entery_updated1.get() + """\n have a nice day """
                  welcoming_email('Feedback',result[0], email_msg)
                  entery_id1.delete(0, END)
                  entery_updated1.delete(0, END)
                  updateoptions1.set('Choose')
      ...
      elif updateoptions1.get() == "Address":

         try:
            if messagebox.askyesno(title='Confirmation', message='Do you really want to update ?'):
               sql = "UPDATE workers SET ADDRESS = '%s' WHERE ID = '%d'" % (entery_updated1.get(), int(wk_id.get()))
               ...
               cursor.execute(sql)
               result = cursor.fetchone()
               email_msg = """ Hello mate , \n  your  new adress is """ + entery_updated1.get() + """ \n have a nice day """
               welcoming_email('Feedback',result[0], email_msg)
               entery_id1.delete(0, END)
               entery_updated1.delete(0, END)
               updateoptions1.set('Choose')
...
def delete_worker():
   ...
   if check:
      try:
         ...
         cursor.execute(sql)
         result = cursor.fetchone()
         email_msg = """ Hello mate ,\n  thank you for your service in our hotel we hope you have had a great time with us we all wish you a bright future"""
         welcoming_email('Feedback',result[0], email_msg)
...
f1=Frame(window, bg="#ffffff") # Home page
lof.append(f1)
...
#============================================= main manu =================================
Label(f1,text='Home page',font = ("Forte", 50,"bold"), bg = "#9e6960",fg="#ffffff").place(relx=0, rely=0, relheight=0.15, relwidth=1)
...
Button(f6,text='Home page',font = ("Forte", 25), bg = "#9e6960",fg="#ffffff",activeforeground="#ffffff",activebackground="#9e6960",command=lambda : show(f1)).place(relx=0.25, rely=0.88, height=90, width=200)
...
Button(f8,text='Home page',font = ("Forte", 25), bg = "#9e6960",fg="#ffffff",activeforeground="#ffffff",activebackground="#9e6960",command=lambda : show(f1)).place(relx=0.25, rely=0.88, height=90, width=200)
...
Button(root, text='Go back', font=("Forte", 25), bg="#9e6960", fg="#ffffff",command=lambda : show(f1)).\
...
updatelist = ["Room number", 'number of days', 'Bill update']

updateoptionss = ttk.Combobox(root, values=updatelist, state="readonly", height=6, font=("Arial", 12))
...
fieldnames = ['Name', 'Room Number', 'Registered days', 'Payment', 'Bill']
# ...............table headings............
for i in range(5):
...
#============================================='Update workers info ' ======================================================
Label(f7,text='update Worker ',font = ("Forte", 35), bg = "#9e6960",anchor='w',fg="#ffffff").place(relx=0, rely=0, relheight=0.12, relwidth=1)

Label(f7,text="Id of the worker :",anchor="e",bg="#ffffff", font=("prototype",20)).place(relx=0, rely=0.15, height=40, width=320)
wk_id=StringVar()
entery_id1=Entry(f7,font = ("Arial", 20),borderwidth=2,textvariable=wk_id)
entery_id1.place(relx=0.37, rely=0.15,height=34, width=200)

Label(f7,text="update options :",anchor="e",bg="#ffffff", font=("prototype",20)).place(relx=0, rely=0.20, height=40, width=320)
updatelist=['Salary','phone number','Address','Email']
opt=StringVar()
updateoptions1=ttk.Combobox(f7,values=updatelist,state="readonly",height=6,font = ("Arial", 20),textvariable=opt)
updateoptions1.place(relx=0.37,  rely=0.20,height=34, width=230)
updateoptions1.set('Choose')

wk_update_entery=StringVar()
entery_updated1=Entry(f7,font = ("Arial", 20),borderwidth=2,textvariable=wk_update_entery)
entery_updated1.place(relx=0.30, rely=0.35,height=34, width=400)
...
Button(f7,text='Home page',font = ("Forte", 25), bg = "#9e6960",fg="#ffffff",activeforeground="#ffffff",activebackground="#9e6960",command=lambda : show(f1)).place(relx=0.25, rely=0.88, height=90, width=200)
